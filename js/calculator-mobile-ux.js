 ...
 */
 ...
 if (!bar) return;
 var stickyY = window.scrollY || window.pageYOffset || 0;
 ...
 syncAddToCartRow();
 setTimeout(function () {
 bar.classList.add(BAR_VISIBLE);
 // Sticky bar / body padding must not walk the page after refresh restore.
 if (window.WMScrollStable && typeof window.WMScrollStable.restore === 'function') {
 window.WMScrollStable.restore(stickyY);
 } else {
 try { window.scrollTo({ top: stickyY, left: 0, behavior: 'auto' }); } catch (eSticky) { window.scrollTo(0, stickyY); }
 }
 }, 350);
