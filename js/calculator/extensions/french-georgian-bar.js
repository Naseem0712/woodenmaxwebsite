/**
 * Extension for French Aluminium Door with Georgian Bar (35mm Slim Profile)
...
if (typeof PriceCalculatorBase !== 'undefined') {
  // Create extended class for french-georgian-bar door
  class PriceCalculatorFrenchGeorgianBar extends PriceCalculatorBase {
    constructor(productId, productConfig, containerId) {
      ...
      // Max height validation (9 feet)
      this.MAX_HEIGHT_FT = productConfig.maxHeight || 9;
    }
...
Calculated Amount:
- Per Window: ${typeof window.formatPriceFromINR === 'function' ? window.formatPriceFromINR(Math.round(finalPerWindow)) : '\u20B9' + finalPerWindow.toLocaleString('en-IN')}
...
Generated from Live Price Calculator
  ...
  if (typeof createExtensionInitCalculator !== 'undefined') {
    createExtensionInitCalculator('french-georgian-bar', PriceCalculatorFrenchGeorgianBar, 'french-georgian-bar');
  } else {
    ...
    window.initCalculator = async function(productId, containerId = null) {
      if (productId === 'french-georgian-bar') {
        try {
          ...
          const calculator = new PriceCalculatorFrenchGeorgianBar(productId, productData, calcContainerId);
          window[`calculator_${productId}`] = calculator;
          return calculator;
        } catch (error) {
          console.error(`Error initializing french-georgian-bar calculator:`, error);
          return null;
        }
  ...
  // Export
  window.PriceCalculatorFrenchGeorgianBar = PriceCalculatorFrenchGeorgianBar;
}
