...
Cost Breakdown:
- Pergola Panel Cost: ${typeof window.formatPriceFromINR === 'function' ? window.formatPriceFromINR(Math.round(amounts.mainCost)) : '\u20B9' + Math.round(amounts.mainCost).toLocaleString('en-IN')}
  ...
  submitEmailForm(emailBody, userDetails) {
    if (window.EmailSubmitter) {
      window.EmailSubmitter.submit({
        subject: 'New Quote - Ceiling Pergola Louvers',
        message: emailBody,
