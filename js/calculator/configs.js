  ...
  async loadProducts() {
    ...
    try {
      if (this.dataSource === 'json') {
        ...
        const getJsonPath = () => {
          try {
            // Get current script path or page path
            ...
            if (currentScript && currentScript.src) {
              ...
              // Calculate relative path from js/calculator/ to root
              // js/calculator/configs.js -> ../../data/products.json
              ...
              const parts = pathname.split('/').filter(p => p && !p.endsWith('.html') && p !== '');
              
              // Count directory levels from root
              // Root file (index.html) = 0 levels, need data/products.json
              // products/metal-louvers/file.html = 2 levels, need ../../data/products.json
